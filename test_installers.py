...
EXPECTED_TS = [
    'utils.ts', 'hashline.ts', 'impact.ts', 'verify.ts',
    'trace.ts', 'changelog.ts', 'graph.ts', 'lint.ts',
    'refactor.ts', 'rename.ts', 'search.ts',
]

EXPECTED_PY_ENGINES = [
    'hashline.py', 'impact.py', 'verify.py', 'calltrace.py',
    'changelog.py', 'graph.py', 'lint.py', 'refactor.py',
    'rename.py', 'search.py',
]
...
def test_bat_local_copies_plugins():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for fn in EXPECTED_TS:
            check(f'bat local .ts plugin: {fn}',
        ...
        for fn in EXPECTED_TS:
            check(f'bat local .opencode/plugins: {fn}',
                  os.path.exists(os.path.join(proj_dir, '.opencode', 'plugins', fn)))
...
def test_ps1_offline_mode():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for fn in EXPECTED_TS:
            check(f'ps1 offline .ts plugin: {fn}',
                  os.path.exists(os.path.join(opcode_dir, 'plugins', fn)),
                  p.stderr[:200])
        ...
        # engine files
        for fn in EXPECTED_PY_ENGINES:
            check(f'ps1 offline engine: {fn}',
                  os.path.exists(os.path.join(proj_dir, fn)),
                  p.stderr[:200])
...
def test_bat_config_dir_precedence():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        check('bat XDG_CONFIG_HOME takes precedence',
              os.path.exists(os.path.join(opcode_dir_xdg, 'plugins')))
...
def test_ps1_config_dir_precedence():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        check('ps1 XDG_CONFIG_HOME takes precedence',
              os.path.exists(os.path.join(opcode_dir_xdg, 'plugins')))

...
from verify import (
    _read_file, _checksum, _find_line, _count_matches,
    cmd_summary, cmd_context, cmd_contains, cmd_line_check, cmd_replace_verify,
    format_pretty, main,
)
...
class TestFormatPretty(unittest.TestCase):
    def test_summary(self):
        out = format_pretty({'status': 'ok', 'check': 'summary', 'file': 'a.py', 'lines': 10, 'bytes': 42, 'checksum': 'abc123'})
        self.assertIn('[OK]', out)
        self.assertIn('a.py', out)

    def test_fail(self):
        out = format_pretty({'status': 'fail', 'check': 'contains', 'text': 'x', 'found': False, 'line': None, 'count': 0})
